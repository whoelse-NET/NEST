...
namespace Nest.Tests.Integration.Indices
{
	...
	public class AliasTest : IntegrationTests
	{
		...
		public void SimpleAddRemoveAlias()
		{
			...
			var count1 = this._client.Count<dynamic>(new[] { index }, q => q.MatchAll());
			var count2 = this._client.Count<dynamic>(new[] { alias }, q => q.MatchAll());
			Assert.AreEqual(count1.Count, count2.Count);
			r = this._client.RemoveAlias(index, alias);
			count1 = this._client.Count<dynamic>(new[] { index }, q => q.MatchAll());
			count2 = this._client.Count<dynamic>(new[] { alias }, q => q.MatchAll());
		...
		public void SimpleRenameAlias()
		{
			...
			count1 = this._client.Count<dynamic>(new[] { index }, q => q.MatchAll());
			count2 = this._client.Count<dynamic>(new[] { alias }, q => q.MatchAll());
			var count3 = this._client.Count<dynamic>(new[] { renamed }, q => q.MatchAll());
			...
			count1 = this._client.Count<dynamic>(new[] { index }, q => q.MatchAll());
			count3 = this._client.Count<dynamic>(new[] { renamed }, q => q.MatchAll());
